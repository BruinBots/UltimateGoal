...
public class VinceAutonomous extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        initVuforiaNavigation(); //initialize and activate navigation
        ...
        while (opModeIsActive()) {
            ...
            switch (wobbleBox){
                case "1stBox":{ // Drop Zone A
                    ...
                    while (!findRobotPosition()){
                        moveBot(1,0,0,.5);
                    }
                    stopBot();
                    // Align to shoot
                    while (!alignRobotToShoot()){
                        moveBot(-.5,0,0,.5);
                    }
                    stopBot();
                    // Shoot 3 rings
...
// This function drives on a gyro angle (deg) for a defined distance (in)
    private void driveByGyroEncoder(double gyroAngle, double distance){
        double ticks = 0;
        int ppr = 134;  // Pulses per revolution
        double distPerRev = 3.14*4;
        ticks = rightFrontDrive.getCurrentPosition() + (distance/distPerRev) * ppr;

        while (leftFrontDrive.getCurrentPosition() < ticks) {
            moveBot(.5, getError(0), 0, .5);
        }
